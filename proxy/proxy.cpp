...
void setgtserver() {
    ...
    if (res && res->status == 200) {
        ...
        server->port = (var.find("port") ? var.get_int("port") : server->port);
        server->meta = (var.find("meta") ? var.get("meta") : server->meta);
    }
    else {
        std::cerr << "HTTP request failed with status code: " << (res ? res->status : -1) << '\n';
    }
    ...
    if (sethost.is_open()) {
        ...
        std::cerr << "Failed to open hosts file for writing\n";
    }
}
int main() {
#ifdef _WIN32
    ...
    enet_initialize();
    if (server->start()) {
        PRINTS("server & client proxy started successfully\n");
        while (true) {
            server->poll();
        }
    }
    else {
        PRINTC("failed to initialize server or proxy\n");
    }
    delete server;
    enet_deinitialize();
    return 0;
